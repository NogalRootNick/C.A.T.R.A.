...
int servoDirection = 1; // 1 = ascendente, -1 = descendente
int servoAngle = 0;

// Variable para el control de la logica
bool isLidarEnabled = false;

// --- Funciones para la comunicacion por consola ---
void handleSerialCommands() {
    // Lee desde el puerto serial (monitor serial por USB)
    while (Serial.available()) {
        char command = Serial.read();
        if (command == 'E' || command == 'e') {
            // Activar el sistema
            isLidarEnabled = true;
            Serial.println("Comando recibido: 'E' - Sistema LIDAR ACTIVADO.");
        } else if (command == 'D' || command == 'd') {
            // Desactivar el sistema
            isLidarEnabled = false;
            myservo.write(0); // Detener el servo
            Serial.println("Comando recibido: 'D' - Sistema LIDAR DESACTIVADO.");
        }
    }
}
...
void setup() {
    // Inicializa el puerto serial para depuracion (por USB)
    Serial.begin(115200);
    while (!Serial) { delay(10); }
    Serial.println("Inicializando sistema LIDAR, sensores ambientales y NRF24L01...");
    Serial.println("Escribe 'E' para iniciar y 'D' para detener el sistema en el Monitor Serial.");
    // Añadir un pequeño retraso para permitir que los componentes se estabilicen
    delay(500);
...
// --- Inicializacion de los Sensores ---
    ...
    myservo.write(0); // El servo se detiene al inicio
...
// --- Inicializacion NRF24L01 ---
    ...
    Serial.println("Sistema listo. Esperando comando en el Monitor Serial para iniciar.");
}
...
void runLidarAndSensorSystem() {
    ...
    // --- Movimiento del servo para el escaneo ---
    if (isLidarEnabled) {
        servoAngle += servoDirection;
        if (servoAngle >= 180 || servoAngle <= 0) {
            servoDirection *= -1; // Cambia la dirección
        }
...
void loop() {
    handleSerialCommands(); // Siempre revisa si hay comandos en el Monitor Serial
    if (isLidarEnabled) {
