...
void runLidarSystem() {
    ...
    // Al final del ciclo, se envia la senal para que el esclavo se mueva
...
void loop() {
    ...
    // Si es el turno del LIDAR, ejecuta el sistema
    if (isLidarTurn) {
        runLidarSystem();
    } else {
        // De lo contrario, espera la senal del esclavo
        if (slaveSerial.available()) {
            String command = slaveSerial.readStringUntil('\n');
            command.trim();
            if (command.equals("READY")) {
                isLidarTurn = true; // Comienza el proximo turno del LIDAR
